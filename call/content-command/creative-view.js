export function creativeView(r, process, { esc, listHTML, effortLabel }) {
  ...
  if (process?.directions?.directions?.some((d) => d.service_proof))
    html += `<details><summary>المشكلة والحدث وإثبات الخدمة في كل اتجاه</summary>${process.directions.directions.map((d) => `<h3>${esc(d.title)}</h3><div class="detail-grid">${map(d, { problem: "المشكلة التي يعالجها الإعلان", opening_action: "ماذا يحدث عند الافتتاح؟", change: "ما الذي يتغيّر؟", service_proof: "كيف نرى قيمة الخدمة؟", final_reveal: "الكشف النهائي" })}</div>`).join("")}</details>`;
  if (r.quality_assessments?.length)
    html += `<details><summary>مراجعة جودة الفكرة — تقدير تحريري لا ضمان نتائج</summary><p class="small-note">0 = ناقص، 1 = عام، 2 = محدد قابل للاستخدام، 3 = محدد ومبرر. هذه مراجعة نموذج وليست قياس أداء الجمهور.</p>${r.quality_assessments.map((a) => `<article class="scene"><b>${esc({ idea: "جوهر الفكرة", service: "ظهور قيمة الخدمة", evidence: "سلامة الاستدلال", execution: "وضوح التنفيذ" }[a.criterion] || a.criterion)} · ${esc(a.score)}/3</b><p>${esc(a.reason)}</p></article>`).join("")}</details>`;
  if (r.treatment)
